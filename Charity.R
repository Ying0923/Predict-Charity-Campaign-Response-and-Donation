...
# set location for the project and upload the data for the analysis
setwd('C:/Users/Northwestern/MSDS 422/CharityProject/')
...
FVs <- c("reg1","reg2","reg3","reg4","home","chld", "hinc", "genf","wrat", "donr","part")

# Tranform catergorical Vs from numeric to factors
charity[FVs] <- lapply(charity[FVs], factor)

summary(charity$part)
# We have 3 data set: train, test & valid. 
# EDA we will only use the training data.

train<- subset(charity,charity$part=="train")
test<- subset(charity,charity$part=="test")
valid<- subset(charity,charity$part=="valid")

# Catergorical variables sumamry for the train data
summary(train[FVs])
...
# Creat function to summary numeric data
num.fun <- function(x){
  c(min=min(x, na.rm=TRUE),
    Quantile = quantile(x, probs=0.01, na.rm= TRUE),
    Quantile = quantile(x, probs=0.05, na.rm= TRUE),
    Quantile = quantile(x, probs=0.25, na.rm= TRUE),
    Mean = mean(x, na.rm= TRUE),
    Median = median(x, na.rm= TRUE),
    Quantile = quantile(x, probs=0.75, na.rm= TRUE),
    Quantile = quantile(x, probs=0.95, na.rm= TRUE),
    Quantile = quantile(x, probs=0.99, na.rm= TRUE),
...
num_summary<- sapply(num_data,num.fun)
# write.csv(num_summary, file="num_summary_train.csv")

### Histogram - without tranformation

par(mfrow= c(3,4))
for (col in colnames(num_data[,-1])){
  hist(na.omit(num_data[,col]), main=col, xlab="",  col="gray")
}
...
par(mfrow= c(3,4))
for (col in colnames(num_data[,-1])){
  hist(log(na.omit(num_data[,col])), main=col, xlab="log",  col="gray")
}
...
par(mfrow= c(3,4))
for (col in colnames(num_data[,-1])){
  hist(sqrt(na.omit(num_data[,col])), main=col, xlab="sqrt",  col="gray")
}


### People made donations - without transformation

par(mfrow= c(3,4))
for (col in colnames(num_data[,-1])){
  hist(na.omit(subset(num_data, num_data$damt>0)[,col]), main=col, xlab="log",  col="gray")
}


### Numeric by donr

# inca, plow, npro, tgif stands out
# rest does not seems had distribution difference between donr 

data.frame(aggregate(train[, colnames(num_data[,-1])], list(train$donr), median))
...
AOV_test <- function(x)
{
  summary(aov(x~donr,  data = train))[[1]][["Pr(>F)"]]
}

sapply(num_data[,-1], AOV_test)   
...
### Boxplot color by donr

boxplot(train[c("avhv", 'incm', 'inca', 'plow', 'npro', 'tgif','lgif', 'rgif', 'tdon', 'tlag', 'agif')], 
        las = 2, main = "Boxplot of Numerical Variables" ) #col = rainbow(11))

library(ggplot2)
ggplot(train, aes(y = avhv, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "avhv") +
  theme_minimal() +
...
ggplot(train, aes(y = incm, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "incm") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = inca, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "inca") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = plow, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "plow") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = npro, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "npro") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = tgif, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "tgif") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = lgif, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "lgif") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = rgif, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "rgif") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = tdon, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "tdon") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = tlag, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "tlag") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
ggplot(train, aes(y = agif, x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "agif") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
for( p in c("reg1", "reg2" ,"reg3", "reg4", "home", "chld", "hinc", "genf", "wrat" )){
  print(p)
  with(train,print(table(donr,get(p))))
}


### EDA For Donor Amount

# boxplots of donor amount by factor
par(mfrow= c(2,5))
for( p in c("reg1", "reg2" ,"reg3", "reg4", "home", "chld", "hinc", "genf", "wrat" )){
  boxplot(damt~get(p),subset(train, train$donr=="1"),main = paste("damt vs" , p))
}

### Scatterplot 

# related to household 
pairs(damt~avhv+incm+inca+plow, data=subset(train, train$donr=="1"))

# related to dolloar amount & promotions
pairs(damt~npro+tgif+lgif+rgif+tdon+tlag+agif, data=subset(train, train$donr=="1"))

library(ggplot2)
library(gridExtra)

ggplot(data=train, aes(x=log(npro), y=log(tgif/agif), color=donr)) +
  geom_point(pch=16, size=2) +
  geom_smooth(method="lm",  se = FALSE, color="black",  size=1.2, linetype=1) +
  labs(title="Lifetime Promotion and $ Gifts Color by Donor", x="# Lifetime Promotions", y="$ Lifetime Gifts") +
  theme( 
...
ggplot(train, aes(y = log(tgif/agif), x = donr, color = donr)) +
  geom_boxplot() +
  labs(x = "donr", y = "tlag") +
  theme_minimal() +
  theme(axis.title.y = element_text(vjust = 1),
...
library(rpart)
tree <- rpart(donr~.-ID - damt, method="class", data=train)
